...
export const generateAccountNumber = () => {
  //let's make the first 2 digits of this 10 digits account number unique
  // this line suggests that the first 2 digits must be 20, 21, or 22
  const prefixDigits = ["20", "21", "22"];
  const digits = prefixDigits[Math.floor(Math.random() * prefixDigits.length)];
  // console.log(digits);

  //now let's get the remaining 8 digits
  let extraDigits = "";
  for (let i = 0; i < 8; i++) {
